                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div style="float:right;">
                                <a href="{{ route('reseller.add') }}" class="btn btn-primary btn-block">Add</a>
                            </div>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
    ...
    <script type="text/javascript">
        $(function() {
            ...
            var table = $('.data-table').DataTable({
                ...
                "lengthChange": false,
                "autoWidth": false,
                ajax: "{{ secure_url('panel/reseller') }}",  // Use secure_url for HTTPS
                'select': {
                    'style': 'multi'
                },
                dom: 'Bfrtip',
                buttons: [{
                        ...
                        action: function(e, dt, node, config) {
                            if (confirm('Are you sure do you want to disable selected items ?')) {
                                ...
                                $form = $("<form action='{{ route('reseller.bulk_disable') }}' method='post'></form>");
                                $form.append('@csrf');
                    ...
                    },
                    {
                        ...
                        action: function(e, dt, node, config) {
                            if (confirm('Are you sure do you want to Activate selected items ?')) {
                                var rows_selected = table.column(0).checkboxes.selected();
                                $form = $("<form action='{{ route('reseller.bulk_activate') }}' method='post'></form>");
                                $form.append('@csrf');
                                $.each(rows_selected, function(index, rowId) {
                                    $form.append(
                                        $('<input>')
